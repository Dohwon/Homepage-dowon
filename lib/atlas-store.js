...
function validateGraphEdge(record) {
  ...
  edge.evidence_links = edge.evidence_links.map((record) => {
    ...
    if (!isNonEmptyString(link.label) || !isNonEmptyString(link.url)) {
...
async function loadBundle(bundleDir, manifest) {
  ...
  const graph = validateGraph(nodes, edges, projects);
  const bundle = {
...
function filterGraph(graph, visibleIds) {
  ...
  const reachable = new Set(visibleProjectNodeIds);
  const pending = [...visibleProjectNodeIds];
  ...
  const prunableKinds = new Set(["Artifact", "Technology", "KnowledgeTag"]);
  const nodes = graph.nodes
    .filter((node) => {
      if (hiddenProjectNodeIds.has(node.id)) return false;
      return !prunableKinds.has(node.kind) || reachable.has(node.id);
    })
