const fsp = require("node:fs/promises");
const path = require("node:path");
const { assertSafePublicValue } = require("./public-content-policy");
const { validatePublicEvidenceUrlLiteral } = require("./public-evidence-url");

const PUBLIC_OVERRIDE_FIELDS = new Set([
  "name",
  "summary",
  "highlights",
  "links",
  "manualOrder",
  "pinned",
  "preview"
]);
const V1_PROJECT_SECTION_FILES = {
  buildStory: "build-story.md",
  decisions: "decisions.md",
  rollbacks: "rollbacks.md",
  visualMap: "visuals/problem-solving.svg"
};
const HEAVY_PROJECT_FIELDS = new Set([
  ...Object.keys(V1_PROJECT_SECTION_FILES),
  "article",
  "timeline",
  "evidence",
  "systemMap",
  "systemMapData",
  "visuals",
  "coverData",
  "captures",
  "captureData"
]);
const PROJECT_ID = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;
const GRAPH_NODE_KINDS = new Set([
  "KnowledgeFocus", "KnowledgeDomain", "KnowledgeTag", "Project", "Technology", "Artifact"
]);
const GRAPH_EDGE_KINDS = new Set([
  "HAS_FOCUS", "FOCUS_HAS_TAG", "HAS_SUBTAG", "HAS_TAG", "USES_TECH",
  "PRODUCES_ARTIFACT", "ARTIFACT_HAS_TAG", "EVOLVED_FROM", "VALIDATES", "DEPLOYS",
  "REUSES_COMPONENT"
]);
const LEGACY_GRAPH_NODE_KINDS = new Set([
  "project", "domain", "problem", "pattern", "technology", "outcome"
]);
const LEGACY_GRAPH_EDGE_KINDS = new Set(["tag-membership", "project-similarity"]);
const TOPIC_KINDS = new Set(["domain", "problem", "pattern", "technology", "outcome"]);
const TAG_LIMITS = {
  domain: [1, 2],
  problem: [1, 3],
  pattern: [1, 3],
  technology: [0, 12],
  outcome: [1, 2]
};
const PUBLIC_PROJECT_FIELDS = new Set([
  "id",
  "name",
  "lifecycle",
  "publication",
  "summary",
  "tags",
  "outcome",
  "aliases"
]);
const PUBLIC_RECORD_FIELDS = {
  node: new Set(["id", "label", "kind", "url", "summary"]),
  edge: new Set(["id", "source", "target", "kind", "weight", "evidence_links"]),
  topic: new Set(["kind", "label", "project_ids"]),
  change: new Set(["project_id", "event_id", "title", "stage", "context", "decision", "outcome", "date"]),
  search: new Set(["id", "project_id", "title", "body", "url"])
};
const LEGACY_GRAPH_RECORD_FIELDS = {
  node: new Set(["id", "label", "kind"]),
  edge: new Set(["source", "target", "kind", "weight", "reasons"])
};
const EMPTY_BUNDLE = {
  formatVersion: 2,
  version: "unavailable",
  projects: [],
  graph: { nodes: [], edges: [] },
  topics: [],
  changelog: [],
  searchIndex: []
};

async function readJson(filePath) {
  return JSON.parse(await fsp.readFile(filePath, "utf8"));
}

async function readOptionalText(filePath) {
  try {
    return await fsp.readFile(filePath, "utf8");
  } catch (error) {
    if (error.code === "ENOENT") return undefined;
    throw error;
  }
}

async function readOptionalJson(filePath) {
  const value = await readOptionalText(filePath);
  return value === undefined ? undefined : JSON.parse(value);
}

function copyPublicRecord(record, allowedFields, errorCode) {
  if (!record || typeof record !== "object" || Array.isArray(record)) throw new Error(errorCode);
  for (const field of Object.keys(record)) {
    if (!allowedFields.has(field)) throw new Error(errorCode);
  }
  return structuredClone(record);
}

function copyPublicRecords(records, allowedFields, errorCode) {
  if (!Array.isArray(records)) throw new Error(errorCode);
  return records.map((record) => copyPublicRecord(record, allowedFields, errorCode));
}

function isNonEmptyString(value) {
  return typeof value === "string" && value.length > 0;
}

function isStringArray(value, { minimum = 0, maximum = Number.POSITIVE_INFINITY } = {}) {
  return Array.isArray(value)
    && value.length >= minimum
    && value.length <= maximum
    && value.every(isNonEmptyString);
}

function validatePublicCover(record, projectId) {
  const cover = copyPublicRecord(
    record,
    new Set(["alt", "caption", "content_type", "content_hex"]),
    "invalid_atlas_cover"
  );
  if (!isNonEmptyString(cover.alt)
    || !isNonEmptyString(cover.caption)
    || !new Set(["image/png", "image/jpeg", "image/webp"]).has(cover.content_type)
    || !isNonEmptyString(cover.content_hex)
    || !/^[0-9a-f]+$/.test(cover.content_hex)
    || cover.content_hex.length % 2 !== 0) {
    throw new Error("invalid_atlas_cover");
  }
  const bytes = Buffer.from(cover.content_hex, "hex");
  if (!bytes.length || bytes.length > 2 * 1024 * 1024 || bytes.toString("hex") !== cover.content_hex) {
    throw new Error("invalid_atlas_cover");
  }
  const signatureMatches = cover.content_type === "image/png"
    ? bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))
    : cover.content_type === "image/jpeg"
      ? bytes.subarray(0, 3).equals(Buffer.from([255, 216, 255]))
      : bytes.subarray(0, 4).toString("ascii") === "RIFF" && bytes.subarray(8, 12).toString("ascii") === "WEBP";
  if (!signatureMatches) throw new Error("invalid_atlas_cover");
  return {
    cover: {
      src: `/api/atlas/projects/${encodePublicId(projectId)}/cover`,
      alt: cover.alt,
      caption: cover.caption
    },
    coverData: { contentType: cover.content_type, bytes }
  };
}

function validatePublicCaptures(records, projectId) {
  if (!Array.isArray(records)) throw new Error("invalid_atlas_captures");
  const ids = new Set();
  const captures = [];
  const captureData = {};
  for (const record of records) {
    const capture = copyPublicRecord(
      record,
      new Set(["id", "alt", "caption", "role", "content_type", "content_hex"]),
      "invalid_atlas_captures"
    );
    if (!/^[a-z0-9][a-z0-9-]*$/.test(capture.id)
      || ids.has(capture.id)
      || !isNonEmptyString(capture.alt)
      || !isNonEmptyString(capture.caption)
      || !isNonEmptyString(capture.role)
      || !new Set(["image/png", "image/jpeg", "image/webp"]).has(capture.content_type)
      || !isNonEmptyString(capture.content_hex)
      || !/^[0-9a-f]+$/.test(capture.content_hex)
      || capture.content_hex.length % 2 !== 0) {
      throw new Error("invalid_atlas_captures");
    }
    const bytes = Buffer.from(capture.content_hex, "hex");
    if (!bytes.length || bytes.length > 2 * 1024 * 1024 || bytes.toString("hex") !== capture.content_hex) {
      throw new Error("invalid_atlas_captures");
    }
    const signatureMatches = capture.content_type === "image/png"
      ? bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))
      : capture.content_type === "image/jpeg"
        ? bytes.subarray(0, 3).equals(Buffer.from([255, 216, 255]))
        : bytes.subarray(0, 4).toString("ascii") === "RIFF" && bytes.subarray(8, 12).toString("ascii") === "WEBP";
    if (!signatureMatches) throw new Error("invalid_atlas_captures");
    ids.add(capture.id);
    captures.push({
      id: capture.id,
      src: `/api/atlas/projects/${encodePublicId(projectId)}/captures/${encodePublicId(capture.id)}`,
      alt: capture.alt,
      caption: capture.caption,
      role: capture.role
    });
    captureData[capture.id] = { contentType: capture.content_type, bytes };
  }
  return { captures, captureData };
}

function encodePublicId(value) {
  return encodeURIComponent(value).replace(/[!'()*]/g, (character) => (
    `%${character.charCodeAt(0).toString(16).toUpperCase()}`
  ));
}

function validateTags(tags) {
  if (!tags || typeof tags !== "object" || Array.isArray(tags)) return false;
  if (Object.keys(tags).length !== Object.keys(TAG_LIMITS).length) return false;
  return Object.entries(TAG_LIMITS).every(([kind, [minimum, maximum]]) => (
    Object.hasOwn(tags, kind) && isStringArray(tags[kind], { minimum, maximum })
  ));
}

function validatePublicProject(record) {
  const project = copyPublicRecord(record, PUBLIC_PROJECT_FIELDS, "invalid_atlas_project_field");
  const valid = PROJECT_ID.test(project.id)
    && isNonEmptyString(project.name)
    && new Set(["active", "finished"]).has(project.lifecycle)
    && project.publication === "public"
    && isNonEmptyString(project.summary)
    && validateTags(project.tags)
    && (project.outcome === undefined || typeof project.outcome === "string")
    && (project.aliases === undefined || isStringArray(project.aliases));
  if (!valid) throw new Error("invalid_atlas_project");
  return project;
}

function validateGraphNode(record) {
  const node = copyPublicRecord(record, PUBLIC_RECORD_FIELDS.node, "invalid_atlas_graph_node");
  if (!isNonEmptyString(node.id)
    || !isNonEmptyString(node.label)
    || !GRAPH_NODE_KINDS.has(node.kind)
    || typeof node.summary !== "string") {
    throw new Error("invalid_atlas_graph_node");
  }
  try {
    node.url = validatePublicGraphUrl(node.url, { allowEmpty: true });
  } catch {
    throw new Error("invalid_atlas_graph_node");
  }
  return node;
}

function validateGraphEdge(record) {
  const edge = copyPublicRecord(record, PUBLIC_RECORD_FIELDS.edge, "invalid_atlas_graph_edge");
  const valid = isNonEmptyString(edge.id)
    && isNonEmptyString(edge.source)
    && isNonEmptyString(edge.target)
    && GRAPH_EDGE_KINDS.has(edge.kind)
    && Number.isInteger(edge.weight)
    && edge.weight > 0
    && Array.isArray(edge.evidence_links);
  if (!valid) throw new Error("invalid_atlas_graph_edge");
  edge.evidence_links = edge.evidence_links.map((record) => {
    const link = copyPublicRecord(
      record,
      new Set(["label", "url"]),
      "invalid_atlas_graph_edge"
    );
    if (!isNonEmptyString(link.label)
      || hasGenericAbsolutePath(link.label)
      || !isNonEmptyString(link.url)) {
      throw new Error("invalid_atlas_graph_edge");
    }
    try {
      link.url = validatePublicGraphUrl(link.url);
    } catch {
      throw new Error("invalid_atlas_graph_edge");
    }
    return link;
  });
  return edge;
}

function hasGenericAbsolutePath(value) {
  const withoutHttpUrls = value.replace(/https?:\/\/[^\s<>"',;)\]}]+/gi, "");
  return /(^|[^A-Za-z0-9_/\\])\/(?![/>#])[^\s<>"']*/.test(withoutHttpUrls)
    || /(^|[^A-Za-z0-9])[A-Za-z]:[\\/]/.test(withoutHttpUrls);
}

function validateLegacyGraphNode(record) {
  const node = copyPublicRecord(
    record,
    LEGACY_GRAPH_RECORD_FIELDS.node,
    "invalid_atlas_graph_node"
  );
  if (!isNonEmptyString(node.id)
    || !isNonEmptyString(node.label)
    || !LEGACY_GRAPH_NODE_KINDS.has(node.kind)) {
    throw new Error("invalid_atlas_graph_node");
  }
  return node;
}

function validateLegacyGraphEdge(record) {
  const edge = copyPublicRecord(
    record,
    LEGACY_GRAPH_RECORD_FIELDS.edge,
    "invalid_atlas_graph_edge"
  );
  const valid = isNonEmptyString(edge.source)
    && isNonEmptyString(edge.target)
    && LEGACY_GRAPH_EDGE_KINDS.has(edge.kind)
    && Number.isInteger(edge.weight)
    && edge.weight > 0
    && isStringArray(edge.reasons);
  if (!valid) throw new Error("invalid_atlas_graph_edge");
  return edge;
}

function validateLegacyGraph(nodes, edges) {
  return {
    nodes: copyPublicRecords(
      nodes,
      LEGACY_GRAPH_RECORD_FIELDS.node,
      "invalid_atlas_graph_node"
    ).map(validateLegacyGraphNode),
    edges: copyPublicRecords(
      edges,
      LEGACY_GRAPH_RECORD_FIELDS.edge,
      "invalid_atlas_graph_edge"
    ).map(validateLegacyGraphEdge)
  };
}

function validatePublicGraphUrl(value, { allowEmpty = false } = {}) {
  if (allowEmpty && value === "") return value;
  if (typeof value !== "string" || /[\u0000-\u0020\u007F\\]/.test(value)) {
    throw new Error("invalid_atlas_graph_url");
  }
  if (value.startsWith("https://")) {
    validatePublicEvidenceUrlLiteral(value);
    return value;
  }
  if (!value.startsWith("/projects/") || value.includes("#")) {
    throw new Error("invalid_atlas_graph_url");
  }
  const [pathname, query = "", ...rest] = value.split("?");
  const encodedId = pathname.slice("/projects/".length);
  if (!encodedId || encodedId.includes("/") || rest.length > 0) {
    throw new Error("invalid_atlas_graph_url");
  }
  let decodedId;
  let decodedAgain;
  try {
    decodedId = decodeURIComponent(encodedId);
    decodedAgain = decodeURIComponent(decodedId);
  } catch {
    throw new Error("invalid_atlas_graph_url");
  }
  if (encodePublicId(decodedId) !== encodedId
    || decodedAgain !== decodedId
    || decodedId.startsWith("/")
    || decodedId.startsWith("\\")
    || decodedId.split("/").some((segment) => segment === "." || segment === "..")) {
    throw new Error("invalid_atlas_graph_url");
  }
  if (query && !new Set([
    "tab=decisions", "tab=system-map", "tab=build-timeline", "tab=evidence"
  ]).has(query)) {
    throw new Error("invalid_atlas_graph_url");
  }
  return value;
}

function validateGraph(nodes, edges, projects) {
  const validatedNodes = copyPublicRecords(
    nodes,
    PUBLIC_RECORD_FIELDS.node,
    "invalid_atlas_graph_node"
  ).map(validateGraphNode);
  const nodeById = new Map();
  for (const node of validatedNodes) {
    if (nodeById.has(node.id)) throw new Error("invalid_atlas_graph_node");
    nodeById.set(node.id, node);
  }

  const expectedProjects = new Map(projects.map((project) => [
    `project:${encodePublicId(project.id)}`,
    project
  ]));
  const projectNodes = validatedNodes.filter((node) => node.kind === "Project");
  if (projectNodes.length !== expectedProjects.size) throw new Error("invalid_atlas_graph_node");
  for (const node of projectNodes) {
    const project = expectedProjects.get(node.id);
    if (!project
      || node.label !== project.name
      || node.url !== `/projects/${encodePublicId(project.id)}`
      || node.summary !== project.summary) {
      throw new Error("invalid_atlas_graph_node");
    }
  }

  const validatedEdges = copyPublicRecords(
    edges,
    PUBLIC_RECORD_FIELDS.edge,
    "invalid_atlas_graph_edge"
  ).map(validateGraphEdge);
  const edgeIds = new Set();
  const identities = new Set();
  for (const edge of validatedEdges) {
    const identity = `${edge.kind}\u0000${edge.source}\u0000${edge.target}`;
    const expectedId = `${edge.kind.toLowerCase()}:${encodePublicId(edge.source)}:${encodePublicId(edge.target)}`;
    if (edgeIds.has(edge.id)
      || identities.has(identity)
      || edge.id !== expectedId
      || edge.source === edge.target
      || !nodeById.has(edge.source)
      || !nodeById.has(edge.target)) {
      throw new Error("invalid_atlas_graph_edge");
    }
    edgeIds.add(edge.id);
    identities.add(identity);
  }
  return { nodes: validatedNodes, edges: validatedEdges };
}

function validateTopic(record, projectIds) {
  const topic = copyPublicRecord(record, PUBLIC_RECORD_FIELDS.topic, "invalid_atlas_topic");
  const valid = TOPIC_KINDS.has(topic.kind)
    && isNonEmptyString(topic.label)
    && isStringArray(topic.project_ids, { minimum: 1 })
    && topic.project_ids.every((id) => projectIds.has(id));
  if (!valid) throw new Error("invalid_atlas_topic");
  return topic;
}

function validateChange(record, projectIds) {
  const change = copyPublicRecord(record, PUBLIC_RECORD_FIELDS.change, "invalid_atlas_change");
  const valid = projectIds.has(change.project_id)
    && ["event_id", "title", "stage"].every((field) => isNonEmptyString(change[field]))
    && ["context", "decision", "outcome"].every((field) => typeof change[field] === "string")
    && (change.date === undefined || /^\d{4}-\d{2}-\d{2}$/.test(change.date));
  if (!valid) throw new Error("invalid_atlas_change");
  return change;
}

function validateSearchRecord(record, projectIds) {
  const search = copyPublicRecord(record, PUBLIC_RECORD_FIELDS.search, "invalid_atlas_search_record");
  const valid = ["id", "project_id", "title", "url"].every((field) => isNonEmptyString(search[field]))
    && typeof search.body === "string"
    && projectIds.has(search.project_id)
    && search.url.startsWith("/projects/");
  if (!valid) throw new Error("invalid_atlas_search_record");
  return search;
}

function validatePublicEvidence(records) {
  const fields = new Set(["id", "label", "source_type", "observed_at", "url"]);
  const evidence = copyPublicRecords(records, fields, "invalid_atlas_evidence");
  const ids = new Set();
  for (const record of evidence) {
    const valid = /^[a-z0-9][a-z0-9-]*$/.test(record.id)
      && isNonEmptyString(record.label)
      && new Set(["session", "spec", "code", "test", "git", "project_memory"]).has(record.source_type)
      && /^\d{4}-\d{2}-\d{2}T/.test(record.observed_at);
    if (!valid || ids.has(record.id)) throw new Error("invalid_atlas_evidence");
    if (record.url !== undefined) {
      try {
        record.url = validatePublicEvidenceUrlLiteral(record.url);
      } catch {
        throw new Error("invalid_atlas_evidence");
      }
    }
    ids.add(record.id);
  }
  return evidence;
}

function validatePublicTimeline(records) {
  const fields = new Set(["event_id", "date", "title", "context", "decision", "outcome", "stage"]);
  const timeline = copyPublicRecords(records, fields, "invalid_atlas_timeline");
  for (const record of timeline) {
    const valid = /^[a-z0-9][a-z0-9-]*$/.test(record.event_id)
      && /^\d{4}-\d{2}-\d{2}$/.test(record.date)
      && ["title", "stage"].every((field) => isNonEmptyString(record[field]))
      && ["context", "decision", "outcome"].every((field) => typeof record[field] === "string");
    if (!valid) throw new Error("invalid_atlas_timeline");
  }
  return timeline;
}

function validatePublicArticle(record, projectId) {
  const fields = new Set(["project_id", "title", "summary", "orientation", "orientation_evidence_ids", "readiness", "prior_context", "sections", "decision_index"]);
  const article = copyPublicRecord(record, fields, "invalid_atlas_article");
  if (article.project_id !== projectId || !isNonEmptyString(article.title) || !isNonEmptyString(article.summary)
    || !new Set(["ready", "insufficient-evidence", "review-required"]).has(article.readiness)
    || (article.orientation !== undefined && !isNonEmptyString(article.orientation))
    || (article.orientation_evidence_ids !== undefined && !isStringArray(article.orientation_evidence_ids))
    || (article.prior_context !== undefined && !isNonEmptyString(article.prior_context))
    || !Array.isArray(article.sections)) throw new Error("invalid_atlas_article");
  const evidenceIds = new Set();
  const diagramIds = new Set();
  const sectionIds = new Set();
  if (article.orientation_evidence_ids !== undefined) {
    if (new Set(article.orientation_evidence_ids).size !== article.orientation_evidence_ids.length) {
      throw new Error("invalid_atlas_article");
    }
    article.orientation_evidence_ids.forEach((id) => evidenceIds.add(id));
  }
  article.sections = article.sections.map((section) => {
    const item = copyPublicRecord(section, new Set(["id", "title", "section_type", "body", "evidence_ids", "diagrams"]), "invalid_atlas_article");
    const valid = /^[a-z0-9][a-z0-9-]*$/.test(item.id)
      && isNonEmptyString(item.title)
      && new Set(["planning", "decision", "implementation", "validation", "result"]).has(item.section_type)
      && isNonEmptyString(item.body)
      && isStringArray(item.evidence_ids);
    if (!valid || sectionIds.has(item.id) || new Set(item.evidence_ids).size !== item.evidence_ids.length) {
      throw new Error("invalid_atlas_article");
    }
    sectionIds.add(item.id);
    item.evidence_ids.forEach((id) => evidenceIds.add(id));
    if (item.diagrams !== undefined) {
      if (!Array.isArray(item.diagrams)) throw new Error("invalid_atlas_article");
      item.diagrams = item.diagrams.map((diagram) => {
        const value = copyPublicRecord(diagram, new Set(["id", "caption", "alt"]), "invalid_atlas_article");
        if (!/^[a-z0-9][a-z0-9-]*$/.test(value.id) || !isNonEmptyString(value.caption)
          || !isNonEmptyString(value.alt) || diagramIds.has(value.id)) throw new Error("invalid_atlas_article");
        diagramIds.add(value.id);
        return value;
      });
    }
    return item;
  });
  if (article.decision_index !== undefined) {
    if (!Array.isArray(article.decision_index)) throw new Error("invalid_atlas_article");
    const decisionIds = new Set();
    article.decision_index = article.decision_index.map((decision) => {
      const value = copyPublicRecord(decision, new Set(["decision_id", "section_id", "status", "evidence_ids"]), "invalid_atlas_article");
      if (!/^[a-z0-9][a-z0-9-]*$/.test(value.decision_id)
        || !/^[a-z0-9][a-z0-9-]*$/.test(value.section_id)
        || !new Set(["adopted", "revised", "rolled-back", "unresolved"]).has(value.status)
        || !isStringArray(value.evidence_ids)
        || decisionIds.has(value.decision_id)
        || !sectionIds.has(value.section_id)
        || new Set(value.evidence_ids).size !== value.evidence_ids.length) throw new Error("invalid_atlas_article");
      decisionIds.add(value.decision_id);
      value.evidence_ids.forEach((id) => evidenceIds.add(id));
      return value;
    });
  }
  return { article, evidenceIds, diagramIds };
}

function validatePublicSystemMap(record, projectId, sectionIds) {
  const stableId = /^[a-z0-9][a-z0-9-]*$/;
  const kinds = new Set(["actor", "input", "interface", "process", "state", "service", "output", "guardrail"]);
  const systemMap = copyPublicRecord(
    record,
    new Set(["project_id", "map_type", "title", "summary", "nodes", "flows", "decision_links"]),
    "invalid_atlas_system_map"
  );
  if (systemMap.project_id !== projectId || !isNonEmptyString(systemMap.map_type) || !isNonEmptyString(systemMap.title)
    || !isNonEmptyString(systemMap.summary) || !Array.isArray(systemMap.nodes)
    || systemMap.nodes.length < 2 || !Array.isArray(systemMap.flows)
    || systemMap.flows.length < 1 || !Array.isArray(systemMap.decision_links)) {
    throw new Error("invalid_atlas_system_map");
  }
  const nodeIds = new Set();
  systemMap.nodes = systemMap.nodes.map((node) => {
    const value = copyPublicRecord(node, new Set(["id", "label", "kind", "description"]), "invalid_atlas_system_map");
    if (!stableId.test(value.id) || !isNonEmptyString(value.label) || !kinds.has(value.kind)
      || !isNonEmptyString(value.description) || nodeIds.has(value.id)) throw new Error("invalid_atlas_system_map");
    nodeIds.add(value.id);
    return value;
  });
  const flowIds = new Set();
  systemMap.flows = systemMap.flows.map((flow) => {
    const value = copyPublicRecord(flow, new Set(["id", "from", "to", "label"]), "invalid_atlas_system_map");
    if (!stableId.test(value.id) || !nodeIds.has(value.from) || !nodeIds.has(value.to)
      || !isNonEmptyString(value.label) || flowIds.has(value.id)) throw new Error("invalid_atlas_system_map");
    flowIds.add(value.id);
    return value;
  });
  systemMap.decision_links = systemMap.decision_links.map((link) => {
    const value = copyPublicRecord(link, new Set(["node_ids", "section_id", "label"]), "invalid_atlas_system_map");
    if (!isStringArray(value.node_ids) || !value.node_ids.length
      || new Set(value.node_ids).size !== value.node_ids.length
      || value.node_ids.some((id) => !nodeIds.has(id))
      || !sectionIds.has(value.section_id) || !isNonEmptyString(value.label)) {
      throw new Error("invalid_atlas_system_map");
    }
    return value;
  });
  return systemMap;
}

async function loadV2ProjectContent(projectDir, projectId, project) {
  const articleRecord = await readOptionalJson(path.join(projectDir, "article.json"));
  const extras = await Promise.all([
    readOptionalJson(path.join(projectDir, "evidence.json")),
    readOptionalJson(path.join(projectDir, "timeline.json")),
    readOptionalJson(path.join(projectDir, "system-map.json")),
    readOptionalText(path.join(projectDir, "system-map.svg"))
  ]);
  if (articleRecord === undefined) {
    if (extras.some((value) => value !== undefined)) throw new Error("invalid_atlas_v2_project_content");
    return;
  }
  const { article, evidenceIds, diagramIds } = validatePublicArticle(articleRecord, projectId);
  const evidence = extras[0] === undefined ? [] : validatePublicEvidence(extras[0]);
  if ([...evidenceIds].some((id) => !evidence.some((record) => record.id === id))) throw new Error("invalid_atlas_article_evidence");
  project.article = article;
  if (evidence.length) project.evidence = evidence;
  if (extras[1] !== undefined) project.timeline = validatePublicTimeline(extras[1]);
  if ((extras[2] === undefined) !== (extras[3] === undefined)) throw new Error("invalid_atlas_system_map");
  if (extras[2] !== undefined) {
    if (!isNonEmptyString(extras[3]) || !extras[3].includes("<svg")) throw new Error("invalid_atlas_system_map");
    const sectionIds = new Set(article.sections.map((section) => section.id));
    project.systemMapData = validatePublicSystemMap(extras[2], projectId, sectionIds);
    project.systemMap = extras[3];
  }
  if (diagramIds.size) {
    project.visuals = {};
    for (const diagramId of [...diagramIds].sort()) {
      const svg = await readOptionalText(path.join(projectDir, "visuals", `${diagramId}.svg`));
      if (!isNonEmptyString(svg) || !svg.includes("<svg")) throw new Error("invalid_atlas_visual");
      project.visuals[diagramId] = svg;
    }
  }
  const coverRecord = await readOptionalJson(path.join(projectDir, "cover.json"));
  if (coverRecord !== undefined) Object.assign(project, validatePublicCover(coverRecord, projectId));
  const capturesRecord = await readOptionalJson(path.join(projectDir, "captures.json"));
  if (capturesRecord !== undefined) Object.assign(project, validatePublicCaptures(capturesRecord, projectId));
  if (project.captures === undefined && project.cover !== undefined) {
    project.captures = [{ ...project.cover, id: "cover", role: "overview" }];
  }
}

async function loadBundle(bundleDir, manifest) {
  if (!manifest || typeof manifest.version !== "string" || !Array.isArray(manifest.projects)
    || ![1, 2].includes(manifest.format_version)) {
    throw new Error("invalid_atlas_manifest");
  }

  const projects = [];
  for (const id of manifest.projects) {
    if (typeof id !== "string" || !PROJECT_ID.test(id)) throw new Error("invalid_atlas_project_id");
    const projectDir = path.join(bundleDir, "projects", id);
    const record = await readJson(path.join(projectDir, "project.json"));
    if (record.id !== id) throw new Error("atlas_project_id_mismatch");

    const project = validatePublicProject(record);
    if (manifest.format_version === 1) {
      for (const [field, filename] of Object.entries(V1_PROJECT_SECTION_FILES)) {
        const content = await readOptionalText(path.join(projectDir, filename));
        if (content !== undefined) project[field] = content;
      }
    } else {
      await loadV2ProjectContent(projectDir, id, project);
    }
    projects.push(project);
  }

  const [nodes, edges, topics, changelog, searchIndex] = await Promise.all([
    readJson(path.join(bundleDir, "graph", "nodes.json")),
    readJson(path.join(bundleDir, "graph", "edges.json")),
    readJson(path.join(bundleDir, "topics.json")),
    readJson(path.join(bundleDir, "changelog.json")),
    readJson(path.join(bundleDir, "search-index.json"))
  ]);

  const projectIds = new Set(projects.map((project) => project.id));
  const searchIds = new Set();
  const validatedSearch = copyPublicRecords(searchIndex, PUBLIC_RECORD_FIELDS.search, "invalid_atlas_search_record")
    .map((record) => validateSearchRecord(record, projectIds));
  for (const record of validatedSearch) {
    if (searchIds.has(record.id)) throw new Error("invalid_atlas_search_record");
    searchIds.add(record.id);
  }
  const graph = manifest.format_version === 1
    ? validateLegacyGraph(nodes, edges)
    : validateGraph(nodes, edges, projects);
  const bundle = {
    formatVersion: manifest.format_version,
    version: manifest.version,
    projects,
    graph,
    topics: copyPublicRecords(topics, PUBLIC_RECORD_FIELDS.topic, "invalid_atlas_topic").map((topic) => validateTopic(topic, projectIds)),
    changelog: copyPublicRecords(changelog, PUBLIC_RECORD_FIELDS.change, "invalid_atlas_change").map((change) => validateChange(change, projectIds)),
    searchIndex: validatedSearch
  };
  assertSafePublicValue(bundle);
  return bundle;
}

function applyCmsOverride(project, override = {}) {
  const merged = structuredClone(project);
  for (const field of PUBLIC_OVERRIDE_FIELDS) {
    if (!Object.hasOwn(override, field)) continue;
    assertSafePublicValue(override[field]);
    merged[field] = structuredClone(override[field]);
  }
  return merged;
}

function mergeCms(bundle, cms = {}) {
  const hiddenIds = new Set(
    Array.isArray(cms?.meta?.hiddenProjectIds)
      ? cms.meta.hiddenProjectIds.map(String)
      : []
  );
  const overrides = new Map(
    (Array.isArray(cms?.projects) ? cms.projects : [])
      .filter((project) => project && typeof project.id === "string")
      .map((project) => [project.id, project])
  );
  const projects = bundle.projects
    .filter((project) => !hiddenIds.has(project.id))
    .map((project) => applyCmsOverride(project, overrides.get(project.id)));
  const visibleIds = new Set(projects.map((project) => project.id));

  return {
    formatVersion: bundle.formatVersion,
    version: bundle.version,
    projects,
    graph: filterGraph(bundle.graph, visibleIds, bundle.formatVersion),
    topics: bundle.topics
      .map((topic) => ({
        ...structuredClone(topic),
        project_ids: topic.project_ids.filter((id) => visibleIds.has(id))
      }))
      .filter((topic) => topic.project_ids.length > 0),
    changelog: bundle.changelog
      .filter((entry) => visibleIds.has(entry.project_id))
      .map((entry) => structuredClone(entry)),
    searchIndex: bundle.searchIndex
      .filter((entry) => visibleIds.has(entry.project_id))
      .map((entry) => structuredClone(entry))
  };
}

function filterGraph(graph, visibleIds, formatVersion) {
  return formatVersion === 1
    ? filterLegacyGraph(graph, visibleIds)
    : filterKnowledgeGraph(graph, visibleIds);
}

function filterLegacyGraph(graph, visibleIds) {
  const visibleProjectNodeIds = new Set(
    [...visibleIds].map((id) => `project:${encodePublicId(id)}`)
  );
  const hiddenProjectNodeIds = new Set(
    graph.nodes
      .filter((node) => node.kind === "project" && !visibleProjectNodeIds.has(node.id))
      .map((node) => node.id)
  );
  const edges = graph.edges
    .filter((edge) => !hiddenProjectNodeIds.has(edge.source) && !hiddenProjectNodeIds.has(edge.target))
    .map((edge) => structuredClone(edge));
  const connectedNodeIds = new Set(edges.flatMap((edge) => [edge.source, edge.target]));
  const nodes = graph.nodes
    .filter((node) => {
      if (hiddenProjectNodeIds.has(node.id)) return false;
      return node.kind === "project" || connectedNodeIds.has(node.id);
    })
    .map((node) => structuredClone(node));
  return { nodes, edges };
}

function projectIdFromGraphNode(nodeId) {
  if (typeof nodeId !== "string" || !nodeId.startsWith("project:")) return null;
  try {
    return decodeURIComponent(nodeId.slice("project:".length));
  } catch {
    return null;
  }
}

function evidenceOwnerProjectId(url) {
  if (typeof url !== "string") return null;
  const match = /^\/projects\/([^/?#]+)\?tab=evidence$/.exec(url);
  if (!match) return null;
  try {
    return decodeURIComponent(match[1]);
  } catch {
    return null;
  }
}

function hasAllowedEvidenceOwners(edge, visibleIds) {
  const endpointOwners = new Set(
    [projectIdFromGraphNode(edge.source), projectIdFromGraphNode(edge.target)].filter(Boolean)
  );
  return edge.evidence_links.every((link) => {
    const owner = evidenceOwnerProjectId(link.url);
    if (owner === null) return true;
    return visibleIds.has(owner) && (!endpointOwners.size || endpointOwners.has(owner));
  });
}

function filterKnowledgeGraph(graph, visibleIds) {
  const visibleProjectNodeIds = new Set(
    [...visibleIds].map((id) => `project:${encodePublicId(id)}`)
  );
  const hiddenProjectNodeIds = new Set(
    graph.nodes
      .filter((node) => node.kind === "Project" && !visibleProjectNodeIds.has(node.id))
      .map((node) => node.id)
  );
  const candidateEdges = graph.edges
    .filter((edge) => !hiddenProjectNodeIds.has(edge.source)
      && !hiddenProjectNodeIds.has(edge.target)
      && hasAllowedEvidenceOwners(edge, visibleIds))
    .map((edge) => structuredClone(edge));
  const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
  const retainedArtifacts = new Set(
    candidateEdges
      .filter((edge) => edge.kind === "PRODUCES_ARTIFACT"
        && visibleProjectNodeIds.has(edge.source)
        && nodeById.get(edge.target)?.kind === "Artifact")
      .map((edge) => edge.target)
  );
  const retainedTechnologies = new Set(
    candidateEdges
      .filter((edge) => edge.kind === "USES_TECH"
        && visibleProjectNodeIds.has(edge.source)
        && nodeById.get(edge.target)?.kind === "Technology")
      .map((edge) => edge.target)
  );
  const retainedTags = new Set(
    candidateEdges
      .filter((edge) => (
        edge.kind === "HAS_TAG" && visibleProjectNodeIds.has(edge.source)
      ) || (
        edge.kind === "ARTIFACT_HAS_TAG" && retainedArtifacts.has(edge.source)
      ))
      .filter((edge) => nodeById.get(edge.target)?.kind === "KnowledgeTag")
      .map((edge) => edge.target)
  );

  let changed = true;
  while (changed) {
    changed = false;
    for (const edge of candidateEdges) {
      if (edge.kind !== "HAS_SUBTAG"
        || !retainedTags.has(edge.target)
        || nodeById.get(edge.source)?.kind !== "KnowledgeTag"
        || retainedTags.has(edge.source)) continue;
      retainedTags.add(edge.source);
      changed = true;
    }
  }

  const nodes = graph.nodes
    .filter((node) => {
      if (hiddenProjectNodeIds.has(node.id)) return false;
      if (node.kind === "Artifact") return retainedArtifacts.has(node.id);
      if (node.kind === "Technology") return retainedTechnologies.has(node.id);
      if (node.kind === "KnowledgeTag") return retainedTags.has(node.id);
      return true;
    })
    .map((node) => structuredClone(node));
  const keptNodeIds = new Set(nodes.map((node) => node.id));
  const edges = candidateEdges.filter(
    (edge) => keptNodeIds.has(edge.source) && keptNodeIds.has(edge.target)
  );
  return { nodes, edges };
}

function projectList(bundle) {
  return [...bundle.projects]
    .sort(compareProjects)
    .map((project) => {
      const item = structuredClone(project);
      for (const field of HEAVY_PROJECT_FIELDS) delete item[field];
      return item;
    });
}

function compareProjects(left, right) {
  if (Boolean(left.pinned) !== Boolean(right.pinned)) return left.pinned ? -1 : 1;
  const leftOrder = Number.isFinite(Number(left.manualOrder)) ? Number(left.manualOrder) : Number.POSITIVE_INFINITY;
  const rightOrder = Number.isFinite(Number(right.manualOrder)) ? Number(right.manualOrder) : Number.POSITIVE_INFINITY;
  if (leftOrder !== rightOrder) return leftOrder - rightOrder;
  return String(left.id).localeCompare(String(right.id), "en");
}

function projectById(bundle, id) {
  const project = bundle.projects.find((item) => item.id === id);
  if (!project) return undefined;
  const copy = structuredClone(project);
  delete copy.coverData;
  delete copy.captureData;
  return copy;
}

function projectCover(bundle, id) {
  const project = bundle.projects.find((item) => item.id === id);
  if (!project?.coverData) return undefined;
  return {
    contentType: project.coverData.contentType,
    bytes: Buffer.from(project.coverData.bytes)
  };
}

function projectCapture(bundle, id, captureId) {
  const project = bundle.projects.find((item) => item.id === id);
  const capture = project?.captureData?.[captureId];
  if (!capture) return undefined;
  return { contentType: capture.contentType, bytes: Buffer.from(capture.bytes) };
}

function searchBundle(bundle, query) {
  const normalized = String(query || "").trim().toLocaleLowerCase();
  if (!normalized) return [];
  return bundle.searchIndex
    .filter((item) => [item.title, item.body, item.id].some((value) => String(value || "").toLocaleLowerCase().includes(normalized)))
    .sort((left, right) => String(left.project_id).localeCompare(String(right.project_id), "en") || String(left.id).localeCompare(String(right.id), "en"))
    .map((item) => structuredClone(item));
}

function createAtlasStore({ bundleDir, loadCmsContent = async () => ({}) }) {
  if (!bundleDir) throw new Error("atlas_bundle_dir_required");
  if (typeof loadCmsContent !== "function") throw new TypeError("loadCmsContent must be a function");

  let cachedVersion = null;
  let cachedBundle = null;

  async function load() {
    let manifest;
    try {
      manifest = await readJson(path.join(bundleDir, "manifest.json"));
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
      return mergeCms(EMPTY_BUNDLE, await loadCmsContent());
    }
    if (!cachedBundle || cachedVersion !== manifest.version) {
      cachedBundle = await loadBundle(bundleDir, manifest);
      cachedVersion = manifest.version;
    }
    return mergeCms(cachedBundle, await loadCmsContent());
  }

  return {
    bootstrap: async () => {
      const bundle = await load();
      return {
        version: bundle.version,
        projects: projectList(bundle),
        topics: structuredClone(bundle.topics),
        changelog: structuredClone(bundle.changelog)
      };
    },
    project: async (id) => projectById(await load(), String(id)),
    cover: async (id) => projectCover(await load(), String(id)),
    capture: async (id, captureId) => projectCapture(await load(), String(id), String(captureId)),
    graph: async () => structuredClone((await load()).graph),
    search: async (query) => searchBundle(await load(), query)
  };
}

module.exports = {
  createAtlasStore
};
