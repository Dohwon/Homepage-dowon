...
async function handleAtlasApi(req, res, url, store) {
  ...
  const captureMatch = /^\/api\/atlas\/projects\/([^/]+)\/captures\/([^/]+)$/.exec(url.pathname);
  if (req.method === "GET" && captureMatch) {
    const id = decodeURIComponent(captureMatch[1]);
    const captureId = decodeURIComponent(captureMatch[2]);
    const capture = await store.capture(id, captureId);
    if (!capture) sendJson(res, 404, { error: "capture_not_found" });
    else sendImage(res, capture);
    return true;
  }
